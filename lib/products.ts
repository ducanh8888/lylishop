...
const JUNE_PROMOTION_NOTE = `Khuyến mãi tháng 6:
• Ưu đãi mua 3 móc khóa đồng giá 45k: tặng 1 móc khóa túi mù size S cho 3 khách hàng đầu tiên.
• Ưu đãi rủ bạn mua cùng: tặng 1 móc khóa size S trị giá tương đương 45k cho 2 khách hàng bất kỳ đã từng mua hàng và rủ bạn đặt chung.

Ghi chú khuyến mãi:
• Mỗi khách hàng chỉ được sử dụng 1 ưu đãi trong tháng 6.
• Hai chương trình khuyến mãi không áp dụng đồng thời cho cùng một người hoặc cùng một đơn hàng.
• Quà tặng là móc khóa túi mù/size S do LyliShop chọn ngẫu nhiên, không quy đổi thành tiền mặt.
• Ưu đãi áp dụng đến khi hết số lượng quà tặng hoặc khi LyliShop thông báo kết thúc chương trình.`;

function productDescriptionWithTieredPrice(
  title: string,
  bullets: string[],
  extraNote?: string
) {
...
${TIERED_SIZE_AND_PRICE_NOTE}${extraNote ? `\n\n${extraNote}` : ""}`;
}
